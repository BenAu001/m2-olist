"""
pipelines/kendra/batch/run_batch.py
────────────────────────────────────
Batch pipeline runner for Kendra (Geography Analytics).

Usage:
    python pipelines/kendra/batch/run_batch.py               # all steps, full refresh
    python pipelines/kendra/batch/run_batch.py --step meltano
    python pipelines/kendra/batch/run_batch.py --step gcs-to-bq
    python pipelines/kendra/batch/run_batch.py --step dbt

Medallion architecture
──────────────────────
  Step 1  Meltano   : tap-csv → target-gcs → GCS Bronze bucket (olist-bronze-kendra)
...
_CONFIG_PATH  = _PROJECT_ROOT / "dashboards" / "kendra" / "config" / "gcp_config.yaml"
_MELTANO_DIR  = Path(__file__).resolve().parent / "meltano"
...
def _read_gcp_config() -> dict:
    ...
    if not _CONFIG_PATH.exists():
        raise FileNotFoundError(
            ...
            "  Run: cp config/gcp_config_template.yaml dashboards/kendra/config/gcp_config.yaml"
        )
...
def run_meltano():
    ...
    try:
        result = subprocess.run(
            ...
            cwd=str(_MELTANO_DIR), capture_output=True, text=True, timeout=600,
        )
        ...
        log("  Run: pip install meltano  &&  cd pipelines/kendra/batch/meltano && meltano install")
        sys.exit(1)
...
def load_gcs_to_bq():
    ...
    bucket      = cfg.get("gcs_bronze_bucket", "olist-bronze-kendra")
    prefix      = "olist/raw/"
    ...
    if auth_method == "service_account":
        ...
        key_file = _PROJECT_ROOT / key_path
    ...
    log(f"  ✓ Dataset {project_id}.olist_raw ready.")

    blobs = list(gcs_client.list_blobs(bucket, prefix=prefix))
    if not blobs:
        log(f"  ⚠ No files found in gs://{bucket}/{prefix} — did Meltano run?")
        sys.exit(1)
...
def run_dbt():
    log("Step 3/3 — dbt: olist_raw → olist_silver → olist_gold...")
    env = {**os.environ, "DBT_PROJECT_ID": "olist-kendralai"}
    try:
        result = subprocess.run(
            [
                "dbt", "run",
            ...
            cwd=str(_DBT_DIR), capture_output=True, text=True, timeout=600, env=env,
        )
        if result.returncode == 0:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Olist batch pipeline runner — Kendra")
    parser.add_argument(
        "--step",
    ...
    log(f"=== Kendra Batch Pipeline Start (step={args.step}) ===")
    start = time.time()
    ...
    log(f"=== Kendra Batch Pipeline Complete ({elapsed}s) ===")
